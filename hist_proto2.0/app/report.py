...
def generate_experiment_pdf(experiment_id: int, user_id: int) -> io.BytesIO:
    ...
    #  Header banner 
    header_data = [[
        Paragraph("Experiment Report",S["title"]),
    ]]
    ...
    saved_str = (
        exp["saved_at"].strftime("%d %b %Y, %H:%M")
        if exp["saved_at"] else "Not saved"
    )
    created_str = (
        exp["created_at"].strftime("%d %b %Y, %H:%M")
        if exp["created_at"] else "—"
    )
    wt_seq = exp["wt_protein_sequence"] or ""
    wt_display = (wt_seq[:80] + "…") if len(wt_seq) > 80 else wt_seq

    meta_data = [
        [Paragraph("Experiment Name", S["label"]),  Paragraph(exp["experiment_name"], S["value"]),
         Paragraph("Experiment ID",   S["label"]),  Paragraph(str(exp["experiment_id"]), S["value"])],
        [Paragraph("UniProt ID",      S["label"]),  Paragraph(exp["uniprot_id"], S["value"]),
         Paragraph("Created",         S["label"]),  Paragraph(created_str, S["value"])],
        [Paragraph("Saved",           S["label"]),  Paragraph(saved_str, S["value"]),
         Paragraph("Report Date",     S["label"]),  Paragraph(datetime.now().strftime("%d %b %Y, %H:%M"), S["value"])],
        [Paragraph("WT Protein (preview)", S["label"]),
         Paragraph(f'<font name="Courier" size="7">{wt_display}</font>', S["value"]),
         Paragraph("", S["label"]), Paragraph("", S["value"])],
    ]

    col_w = (PAGE_W - 2 * MARGIN) / 4
    meta_table = Table(meta_data, colWidths=[col_w * 0.85, col_w * 1.15, col_w * 0.85, col_w * 1.15])
    ...
    for row in stat_rows:
        row_data = [[
            [Paragraph(label, S["label"]), Paragraph(value, ParagraphStyle(
                "StatVal", fontSize=14, fontName="Helvetica-Bold", textColor=BLUE,
                alignment=TA_CENTER,
            ))]
